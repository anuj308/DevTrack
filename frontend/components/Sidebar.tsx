...
export default function Sidebar({
  ...
  return (
    <aside
      className={`${bgClass} ${collapsed ? 'w-20' : 'w-64'} h-screen border-r fixed left-0 top-24 overflow-y-auto transition-all duration-300`}
    >
            ...
            <div className={`mt-2 space-y-1 ${collapsed ? 'px-0' : ''}`}>
              {lists.map((list) => (
                <button
                  ...
                  className={`flex items-center gap-2 w-full rounded-lg transition text-sm ${
                ...
                >
                  <span className="text-lg">📁</span>
                  {!collapsed && <span className="flex-1 text-left">{list.name}</span>}
                  {!collapsed && (
                    <span
